...
contract TierLibTest is Test {
    ...
    function testValidation() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TierLib.TierInvalidDuration.selector)
        );
        shim.validate(tier);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TierLib.TierTimingInvalid.selector)
        );
        shim.validate(tier);
    ...
    function testJoinChecks() public {
        ...
        // Max supply
        vm.expectRevert(
            abi.encodeWithSelector(TierLib.TierHasNoSupply.selector, 1)
        );
        shim.checkJoin(tier, 1000, alice, 1e18);

        vm.expectRevert(
            abi.encodeWithSelector(
                TierLib.TierInvalidMintPrice.selector,
                0.01 ether
            )
        );
        ...
        tier.gate = Gate({
            gateType: GateType.ERC20,
            contractAddress: address(token),
            componentId: 0,
            balanceMin: 1
        });
        vm.expectRevert(
            abi.encodeWithSelector(GateLib.GateCheckFailure.selector)
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TierLib.TierNotStarted.selector)
        );
        shim.checkJoin(tier, 0, alice, 1e18);
    }
    ...
    function testRenewChecks() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TierLib.TierInvalidRenewalPrice.selector,
                tier.pricePerPeriod
            )
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TierLib.MaxCommitmentExceeded.selector)
        );
        shim.checkRenewal(tier, sub, tier.pricePerPeriod * 2);

        tier = defaults();
        tier.endTimestamp = uint48(
            block.timestamp + 2 * tier.periodDurationSeconds
        );
        vm.expectRevert(
            abi.encodeWithSelector(TierLib.TierEndExceeded.selector)
        );
    ...
    function testTimeRatios() public {
        ...
        uint48 numSeconds = shim.computeSwitchTimeValue(
            toTier,
            fromTier,
            30 days
        );
